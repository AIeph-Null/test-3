...
public class TransformWater : MonoBehaviour
{
    ...
    void Update()
    {
        ageNumber = age.value;
        if (version == 1) { //water 
            ageNumber = (age.value/20.0f)*2 + 2.0f - (age2.value / 320.0f) *2;
            this.transform.position = new Vector3(150.0f, ageNumber, 100.0f);
        }
        if (version == 2) { //big iceberg 
            ageNumber = age.value - (age2.value * 0.015f);
        ...
        if (version == 3) { //small iceberg
            ageNumber = ageNumber + 9.5f - (age2.value); //when -2, get -2, when 12 get 0 
        ...
        if (version == 5)  { //water 2
            ageNumber = -8.0f + (age.value / 20.0f) * 2 - (age2.value / 20.0f) * 2; //-10.79
            this.transform.position = new Vector3(23.17027f, ageNumber-1.0f, -14.86819f);
        }
